...
//         (e.g., `QUERY->begin_class(QUERY, "Warp", "Object");`)
//      2. remove or commment out the line containing QUERY->add_ugen_func()
//      3. that's it; the rest is no different for UGens/non-UGens
//-----------------------------------------------------------------------------
// NOTE once you have built this into a chugin (Warp.chug), here are a few
//      helpful tools for testing / probing / verifying your new chugin!
//
// chuginate also generated a Warp-test.ck boilerplate ChucK program
//      to help test your chugin (see Warp-test.ck for more instructions)
//
...
CK_DLL_CTOR( warp_ctor );
// declaration of chugin desctructor
CK_DLL_DTOR( warp_dtor );

// example of getter/setter
CK_DLL_MFUN( warp_setParam );
CK_DLL_MFUN( warp_getParam );

// for chugins extending UGen, this is mono synthesis function for 1 sample
CK_DLL_TICK( warp_tick );

// this is a special offset reserved for chugin internal data
t_CKINT warp_data_offset = 0;
...
CK_DLL_QUERY( Warp )
{
    ...
    QUERY->begin_class( QUERY, "Warp", "UGen" );

    // register default constructor
    QUERY->add_ctor( QUERY, warp_ctor );
    ...
    QUERY->add_dtor( QUERY, warp_dtor );

    // for UGens only: add tick function
    // NOTE a non-UGen class should remove or comment out this next line
    QUERY->add_ugen_func( QUERY, warp_tick, NULL, 1, 1 );
    // NOTE: if this is to be a UGen with more than 1 channel,
    // e.g., a multichannel UGen -- will need to use add_ugen_funcf()
    // and declare a tickf function using CK_DLL_TICKF

    // example of adding setter method
    QUERY->add_mfun( QUERY, warp_setParam, "float", "param" );
    // example of adding argument to the above method
    QUERY->add_arg( QUERY, "float", "arg" );

    // example of adding getter method
    QUERY->add_mfun( QUERY, warp_getParam, "float", "param" );
    
    // this reserves a variable in the ChucK internal class to store 
    // referene to the c++ class we defined above
    warp_data_offset = QUERY->add_mvar( QUERY, "int", "@w_data", false );
...
CK_DLL_CTOR( warp_ctor )
{
    ...
    OBJ_MEMBER_INT( SELF, warp_data_offset ) = 0;
    
    // instantiate our internal c++ class representation
    Warp * w_obj = new Warp( API->vm->srate(VM) );
    
    // store the pointer in the ChucK object member
    OBJ_MEMBER_INT( SELF, warp_data_offset ) = (t_CKINT)w_obj;
}
...
CK_DLL_DTOR( warp_dtor )
{
    ...
    Warp * w_obj = (Warp *)OBJ_MEMBER_INT( SELF, warp_data_offset );
    // clean up (this macro tests for NULL, deletes, and zeros out the variable)
    CK_SAFE_DELETE( w_obj );
...
CK_DLL_MFUN( warp_setParam )
{
    ...
    RETURN->v_float = w_obj->setParam( arg1 );
}

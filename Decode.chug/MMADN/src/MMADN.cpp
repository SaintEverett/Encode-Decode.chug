...
//         (e.g., `QUERY->begin_class(QUERY, "MMADN", "Object");`)
//      2. remove or commment out the line containing QUERY->add_ugen_func()
//      3. that's it; the rest is no different for UGens/non-UGens
//-----------------------------------------------------------------------------
// NOTE once you have built this into a chugin (MMADN.chug), here are a few
//      helpful tools for testing / probing / verifying your new chugin!
//
// chuginate also generated a MMADN-test.ck boilerplate ChucK program
//      to help test your chugin (see MMADN-test.ck for more instructions)
//
...
CK_DLL_CTOR( mmadn_ctor );
// declaration of chugin desctructor
CK_DLL_DTOR( mmadn_dtor );
// example of getter/setter
CK_DLL_MFUN( mmadn_setParam );
CK_DLL_MFUN( mmadn_getParam );
// for chugins extending UGen, this is mono synthesis function for 1 sample
CK_DLL_TICKF( mmadn_tickf );
// this is a special offset reserved for chugin internal data
t_CKINT mmadn1_data_offset = 0;
...
CK_DLL_QUERY( MMADN )
{
    ...
    QUERY->begin_class( QUERY, "MMADN", "UGen" );

    // register default constructor
    QUERY->add_ctor( QUERY, mmadn_ctor );
...
CK_DLL_CTOR( mmadn_ctor )
{
    ...
    OBJ_MEMBER_INT( SELF, mmadn_data_offset ) = 0;
    
    // instantiate our internal c++ class representation
    MMADN * mmadn_obj = new MMADN( API->vm->srate(VM) );
    
    // store the pointer in the ChucK object member
    OBJ_MEMBER_INT( SELF, mmadn_data_offset ) = (t_CKINT)mmadn_obj;
}
...
CK_DLL_DTOR( mmadn_dtor )
{
    ...
    MMADN * mmadn_obj = (MMADN *)OBJ_MEMBER_INT( SELF, mmadn_data_offset );
    // clean up (this macro tests for NULL, deletes, and zeros out the variable)
    CK_SAFE_DELETE( mmadn_obj );
...
CK_DLL_TICKF( mmadn_tickf )
{
    ...
    if( mmadn_obj ) *out = mmadn_obj->tick( in );

...
public:
    // constructor
    AmbPan3( t_CKFLOAT fs) : // constructor immediately creates necessary variables + calculates constants
    SQRT3(sqrtf(3)), 
    SQRT5(sqrtf(5)),
    SQRT15(sqrtf(15))
    {
        m_azimuth = 0; // azimuth
        m_elevation = 0; // elevation
        compute_gains();  // calculate coordinates even with origin position
        for(int i = 0; i < 16; i++) // map channels 
        {
            m_channel_map[i] = i;
        }
    }
    ...
    void tick( SAMPLE * in, SAMPLE * out, int nframes ) // main tick + sig processing
    {
        for(int f = 0; f < nframes; f++) // how many 
        {
            for(int c = 0; c < 16; c++)
            {
    ...
    float setAzimuth( t_CKFLOAT a ) // set azimuth and compute gains
    {
        m_azimuth = a;
        compute_gains();
        return m_azimuth;
    ...
    float getAzimuth() { return m_azimuth; } // get azimuth
    
    // set parameter example
    float setElevation( t_CKFLOAT e ) // set elevation
    {
        m_elevation = e;
        compute_gains();
        return m_elevation;
    ...
    // get parameter example
    float getElevation() { return m_elevation; } // get elevation
    
    void setChannelMap(Chuck_ArrayInt * map, CK_DL_API api) // channel map 
    {
        t_CKINT len = ck_min(api->object->array_int_size(map), 16); // retrieve array size, if over 16, ignore and use 16
        for(t_CKINT i = 0; i < len; i++) // map array indices 
            m_channel_map[i] = api->object->array_int_get_idx(map,i);
    }
    
private:
    
    void compute_gains() // we know this math
    {
    ...
    // instance data
    t_CKFLOAT m_azimuth; // object specific azimuth
    t_CKFLOAT m_elevation; // elevation
    t_CKFLOAT m_gain[16]; // gains
    t_CKUINT m_channel_map[16]; // channel mapping
...
CK_DLL_DTOR(ambpan3_dtor) // destructor
{
...
CK_DLL_TICKF(ambpan3_tickf) // multichannel tick function
{
    // get our c++ class pointer
    AmbPan3 * c = (AmbPan3 *) OBJ_MEMBER_INT(SELF, ambpan3_data_offset);
 
    // invoke our tick function; store in the magical out variable
    if(c) c->tick(in, out, nframes); // don't write your own tick function, just grab it from the class! (assuming it exists)
